...
label academy_library_quest:
    ...
    # --- Already completed ---
    if store.event_flags.get("academy_lib_manual_found"):
        narrator "You already recovered the sealed manual. The library shelves have nothing left to say on the matter — though the dust, as always, has opinions."
...
label academy_lib_prologue:
    narrator "The academy library smells of dust, varnish, and that particular brand of arrogance that ink develops when it outlives the people who wrote it."

    narrator "You are not here for the atmosphere. The card catalog lists a circulating codex — a sealed manual of practical manager training. Drills, forms, the exact phrasing senior stewards use when they turn a worker's bad habit into something teachable."

    narrator "The copy should sit on an open shelf between {i}Intermediate Ledger Etiquette{/i} and {i}So Your Workers Hate You: A Primer{/i}."

    narrator "It does not."

    narrator "Where the spine should be, there is a gap and a label that matches nothing in the room. Loan status: {i}perpetual{/i}. Possession: {i}ambiguous{/i}."

    narrator "You look at the clerk. The clerk looks at a point slightly above your head, the way people do when they have decided not to be helpful."

    narrator "After what feels like a diplomatic incident conducted entirely in sighs, they slide a routing slip across the counter and tap it twice."

    narrator "The manual carries an embossed wax seal — six positions around a ring. Without the correct {i}spoken{/i} token, the binder stays shut. No amount of pulling, prying, or creative vocabulary will change this."

    narrator "On the margin of the slip, someone has penciled a single note in cramped handwriting: {i}\"Position one is M. Confirmed. The rest will follow if you know where to look.\"{/i}"

    narrator "Below it, in different ink — shakier, more desperate: {i}\"Day five. I have tried MASTER, GOVERN, DIRECT, and STEWARD. None of these have six letters. I may need sleep.\"{/i}"

    narrator "A previous manager, it seems, walked this same path and did not finish it."

    narrator "If brute-forcing the cipher sounds beneath you, a few stationers stock ruled cryptographer's lattices — grids that align seal glyphs to their letters mechanically. The Elite Emporium on the city map sometimes carries them, at a price that could buy patience outright."
...
label academy_lib_act1:
    narrator "You return to the library. The clerk acknowledges your existence with the faintest possible nod — progress, by their standards."

    narrator "The catalog system here was designed by someone who believed that finding information should be a character-building exercise. Cross-references lead to cross-references. Footnotes cite other footnotes. It is, in its way, a monument to bureaucratic spite."
...
label academy_lib_act1_archive:
    window show
    narrator "You spend the morning being sent to the wrong desk. Then the wrong floor. Then back to the first desk, where the same clerk pretends not to recognize you."

    narrator "By afternoon, a tired archivist — the only person in the building who appears to actually {i}like{/i} books — lets you examine a corner of the seal rubric."

    narrator "It is written in a hand so precise it could have been typeset. Two more positions, named plainly: {i}the third tooth reads N, the fifth reads G{/i}."
...
label academy_lib_act1_margins:
    ...
    narrator "{i}\"Asked the archivist directly. She laughed. Actually laughed. Then she said 'third is N, fifth is G' as if that were common knowledge. Perhaps it is. Perhaps I am the only person in this building who doesn't already know.\"{/i}"
...
label academy_lib_act1_vowel_clue:
    narrator "Deeper in the restricted catalog, buried in a memo about non-circulating reserves, someone has underlined one more detail: the seal's {i}closing{/i} tooth — position six — resolves to E."

    narrator "And a marginal note in different ink adds: {i}\"Positions two and four carry the same vowel. Not a stutter — a mirror. The house uses it as a signature.\"{/i}"
...
label academy_lib_act1_vowel_choice:
    ...
    menu:
        ...
        "The vowel is O." if not _wrong_o:
            ...
            narrator "The pattern doesn't match any house token in the reference index. The archivist glances at your notes and looks away with what might be pity."
...
label academy_lib_act1_vowel_correct:
    window show
    narrator "M-A-N-A-G-E. You write it slowly, and something shifts — not in the room, but in the way the word sits on the page. It looks {i}right{/i}. It looks like it belongs."

    narrator "You check it against the routing slip annotations. Position one, M. Two, A. Three, N. Four, A. Five, G. Six, E. Every clue you have gathered aligns."

    narrator "In the margin of the ledger you've been following, the previous manager's final note: {i}\"I never got this far. If you're reading this, good luck with the wax. It doesn't bite, but it doesn't forgive either.\"{/i}"
...
label academy_lib_seal_attempt:
    ...
    if academy_lib_has_cipher_lattice():
        narrator "You approach the proving desk with the ruled lattice in hand. The clerk's gaze drops to it, then back to the seal, with the resigned expression of someone watching a puzzle solve itself."
...
label academy_lib_act2:
    narrator "The proving desk waits for you. The seal sits under a brass lamp, wax softened just enough by the heat to show its fine lines — six teeth around a ring, each one a position."

    if store.event_flags.get("academy_lib_ready_decrypt"):
        narrator "You know all six letters. The question is no longer {i}what{/i} to say, but whether you can say it correctly — one word, no hesitation, spoken as if you meant it."
    else:
...
label academy_lib_act2_loop:
    ...
    menu:
        "Use the lattice." if academy_lib_has_cipher_lattice():
            ...
            narrator "The house token assembles itself beneath the lattice like a word that was always there, waiting for someone with the right tool and the good sense to buy it."
        ...
        "Speak the token.":
            ...
            if store.academy_lib_seal_attempts_today == 1:
                ...
                window show
                narrator "Wrong again. The seal sits there with the serene indifference of something that has all the time in the world and knows you don't."
                ...
                window show
                narrator "The clerk reaches across the desk and slides the seal away from you with one finger. They do not speak. They do not need to."
...
label academy_lib_seal_success:
    narrator "The fit is correct. Something inside the binder clicks — not loudly, not dramatically, but with the quiet certainty of a lock that has been waiting for the right word."

    narrator "The latch yields. Inside: not spells, not secrets — {i}protocol{/i}. Pages dense with practical instruction, margins crowded with the kind of knowledge that senior house managers swear they {i}just know{/i}, as if competence were atmospheric and not learned."
...
label academy_lib_epilogue:
    narrator "You slip the codex into your coat like contraband — which, in a sense, it is. Practical knowledge has always been the most closely guarded kind."

    narrator "On the last page, tucked into the binding, a final note from your predecessor: {i}\"If you're reading this, you're better than me. Or richer. Either way, do something useful with it.\"{/i}"

    narrator "From now on, when you sit down with your workers, you can structure real training sessions — with the confidence of someone who finally owns the terminology and isn't afraid to use it."
