...
- Rest jobs are handled as a dedicated branch in daily processing.
...
- Choice options can be trait-gated (`required_traits` / `excluded_traits`) and either hidden or shown as locked.
...
- `required_building_traits`: event appears only if the matching owned building contains all listed traits/tags in `building["traits"]`.
...
## Academy / Training
- Training jobs can generate skill-use growth and recurring costs.
